...
extern int valid_sudoku2(sudoku *s) {
    int x, y, a, b, res, i, blocks;

    int r[SUDOKU_SIZE] = {0};
    int c[SUDOKU_SIZE] = {0};
    int block[SUDOKU_SIZE] = {0};
    blocks = sqrt(SUDOKU_SIZE);
    ...
    for (a = 0; a < blocks; ++a) {
        for (b = 0; b < blocks; ++b) {
            ...
            memset(block, 0, SUDOKU_SIZE * sizeof(int));
            i = 0;
...
extern int valid_sudoku(sudoku *s) {
    int i, j, k, l;
    int num;
    int offset_k, offset_l;
    int row[SUDOKU_SIZE] = {0};
    int col[SUDOKU_SIZE] = {0};

    for (i = 0; i < SUDOKU_SIZE; ++i) {

        memset(row, 0, sizeof(int) * SUDOKU_SIZE);
        memset(col, 0, sizeof(int) * SUDOKU_SIZE);

        for (j = 0; j < SUDOKU_SIZE; ++j) {

            if (s->playfield[i][j] != '-') {
                num = s->playfield[i][j] - '1';
    ...
    for (i = 0; i < SUDOKU_SIZE; ++i) {
        ...
        for (k = 0; k < SQUARE_SIZE; ++k) {
            for (l = 0; l < SQUARE_SIZE; ++l) {
                offset_k = k + (i / SQUARE_SIZE) * SQUARE_SIZE;
                offset_l = (l + i * SQUARE_SIZE) % SUDOKU_SIZE;

                if (s->playfield[offset_k][offset_l] == '-') continue;
                num = s->playfield[offset_k][offset_l] - '1';
                if (row[num]) {
                    return 0;
                }
                row[num] = 1;
